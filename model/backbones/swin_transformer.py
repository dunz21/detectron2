    ...
    def init_weights(self, pretrained=None):
        ...
        if pretrained is None:
            ...
            for table_key in relative_position_bias_table_keys:
                ...
                table_current = self.state_dict()[table_key.replace('base.', '', 1)]
            ...
            def modify_state_dict(state_dict, num_layers_to_remove):
                # Remove the last 'num_layers_to_remove' layers
                keys = list(state_dict.keys())
                reduced_state_dict = {k: state_dict[k] for k in keys[:-num_layers_to_remove]}

                # Remove 'base.' prefix from the keys
                modified_state_dict = {}
                for key in reduced_state_dict:
                    new_key = key.replace('base.', '') if key.startswith('base.') else key
                    modified_state_dict[new_key] = reduced_state_dict[key]

                return modified_state_dict
            filtered_state_dict = modify_state_dict(state_dict, 6)

            res = self.load_state_dict(filtered_state_dict, False)
